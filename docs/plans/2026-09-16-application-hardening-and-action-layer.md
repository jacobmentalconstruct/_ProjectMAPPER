...
(entry approved 2026-09-24; steps 8.1–8.2 complete, 331 tests; 8.3 next).
...
| Testing | Phase 8 step 8.2: 331 tests pass on Python 3.10, 3.13 and 3.14 (exit codes captured; first runs on 3.14 and 3.13 hit the Tk start-up flake, see Acceptance evidence). Phase 7 acceptance: 271 regressions, plus two explicit benchmarks and a measured minimum-size layout probe. Development dependencies are declared in `pyproject.toml` (`.[dev]`). | Whole-application acceptance in Phase 8; coverage for Phase 9. |
...
| Acceptance evidence | Audited at Phase 8 entry against section 14. Step 8.1 closed gaps G1–G6 with tests (fixing defects D1 and D2). Step 8.2 mapped every desktop control, key binding, menu entry and close box to a test that drives it (`docs/ui-map.md`). Still open: the tracked final acceptance report, and three unexplained test-environment events: the intermittent Tk start-up read failure (on 2026-09-25 also seen as "no such file or directory" for an existing `ttk/menubutton.tcl`), one silent 3.10 run, and one baseline-copy layout failure. | Explain the unexplained events (decision E4, step 8.3); write `docs/acceptance/v1.0.0.md`. |
